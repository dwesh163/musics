	...
	return (
		<div className="w-full h-full p-4 sm:p-6 bg-neutral-900">
			<div className="max-w-7xl mx-auto">
				<div className="flex flex-row gap-4 sm:gap-6 mb-6 items-center">
					...
					<div className="flex flex-col justify-center text-left space-y-2">
						<h1 className="text-2xl sm:text-3xl font-bold text-white line-clamp-2">{album.data.name}</h1>
						<div className="text-gray-400 space-y-1">
							<div className="flex items-center gap-2">
								{album.data.artists.map((artist, index) => (
									<>
										<Link key={artist.id} href={`/artist/${artist.id}`} className="text-sm sm:text-base hover:underline hover:text-white">
											{artist.name}
										</Link>
